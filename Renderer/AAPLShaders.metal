...
struct FragmentShaderArguments {
    array<texture2d<float>, 100000> exampleTextures  [[ id(AAPLArgumentBufferIDExampleTextures)  ]];
...
float4 getTextureValue(thread const uint32_t samplerId, thread const float2& uv, thread const array<texture2d<float>, 100000> SceneTextures)
{
	constexpr sampler textureSampler (mag_filter::linear,
									  min_filter::linear);
	return SceneTextures[samplerId].sample(textureSampler, uv);
}
               ...
               device FragmentShaderArguments & fragmentShaderArgs [[ buffer(AAPLFragmentBufferIndexArguments) ]])
{
    ...
    if(in.texCoord.x < 0.5)
    {
        ...
        for(uint32_t textureToSample = 0; textureToSample < 32; textureToSample++)
        {
			float4 textureValue = getTextureValue(textureToSample, in.texCoord, fragmentShaderArgs.exampleTextures);
